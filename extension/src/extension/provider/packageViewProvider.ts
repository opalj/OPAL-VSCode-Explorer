...
export class PackageViewProvider implements vscode.TreeDataProvider<OpalNode> {
  private _onDidChangeTreeData: vscode.EventEmitter<OpalNode | undefined> =
    new vscode.EventEmitter<OpalNode | undefined>();
  readonly onDidChangeTreeData: vscode.Event<OpalNode | undefined> =
  ...
  public getChildren(p?: OpalNode): vscode.ProviderResult<OpalNode[]> {
    if (p) {
      ...
      const p = this.setOpalNodeTree(this._classDAO.classes);
      if (p) {
  ...
  public setOpalNodeTree(classes: ClassFile[]): TreeItem {
    ...
    classes.forEach(async (target) => {
      const fqnParts = target.fqn.split("/");
  ...
  private addNode(
    ...
    const parent = this.searchNode(opalNode as OpalNode, fqnParts[i]);
    if (parent !== null) {
      ...
      if (fqnParts[i] !== undefined) {
        const type = i === fqnParts.length - 1 ? "leaf" : "node";
        const newNode = new OpalNode(
          fqnParts[i],

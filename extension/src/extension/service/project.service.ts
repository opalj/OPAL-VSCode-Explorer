...
export class ProjectService {
  ...
  async unLoad() {
    return request(
      this.serverUrl +
        "/opal/project/delete/" +
        encodeURIComponent(this._projectId)
    );
  }
  ...
  async getOPALLoadMessage(config: any) {
    ...
    const targets = this._classDAO.getFsPaths();
    const libraries = await this.addLibraries(config.libFolders);
    return {
      projectId: projectId,
      targets: targets,
      libraries: libraries,
  ...
  async getLibrariesInWorkspace(): Promise<string[]> {
    const jarURIs = await workspace.findFiles("**/*.jar");
    const paths: string[] = [];
